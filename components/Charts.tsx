...
export function VenueBarChart({data,unit='usd'}:{data:Row[];unit?:'usd'|'bp'|'pct'|'count'}){if(!data.length)return <div className="emptyViz">No reliable data available</div>;return <div className="chartBox"><ResponsiveContainer width="100%" height="100%"><BarChart data={data} margin={{top:12,right:10,left:2,bottom:4}} barCategoryGap="38%"><CartesianGrid vertical={false} stroke="#edf0f2"/><XAxis dataKey="label" axisLine={false} tickLine={false} tick={{fontSize:11,fill:'#606770'}} dy={8}/><YAxis axisLine={false} tickLine={false} width={62} tick={{fontSize:10,fill:'#8b929a'}} tickFormatter={(v)=>unit==='usd'?usd(v):unit==='bp'?bp(v):unit==='count'?Number(v).toLocaleString():`${Number(v).toFixed(0)}%`}/><Tooltip content={<Tip unit={unit}/>} cursor={{fill:'#f6f7f8'}}/><Bar dataKey="value" radius={[3,3,0,0]} maxBarSize={58}>{data.map((r,i)=><Cell key={i} fill={venueColor(r.label)}/>)}</Bar></BarChart></ResponsiveContainer></div>}

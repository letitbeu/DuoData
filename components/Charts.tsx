...
function Tip({active,payload,unit='usd'}:any){if(!active||!payload?.length)return null;const p=payload[0].payload;const v=Number(payload[0].value||0);return <div className="chartTooltip"><strong>{p.label}</strong>{p.venue&&<span>{p.venue}</span>}<b>{unit==='usd'?usd(v):unit==='bp'?bp(v):unit==='pct'?v.toFixed(1)+'%':unit==='count'?Math.round(v).toLocaleString():unit==='index'?v.toFixed(0):pct(v)}</b></div>}
...
function SingleLineYAxisTick({x=0,y=0,payload,maxChars=32}:any){
  const full=String(payload?.value??'')
  const label=truncateLabel(full,maxChars)
  return <g transform={`translate(${x},${y})`}><title>{full}</title><text x={-8} y={0} dy={4} textAnchor="end" fill="#59616a" fontSize={10.5} style={{whiteSpace:'nowrap'}}>{label}</text></g>
}

export function VenueBarChart({data,unit='usd',maxValue}:{data:Row[];unit?:'usd'|'bp'|'pct'|'count'|'index';maxValue?:number}){if(!data.length)return <div className="emptyViz">No reliable data available</div>;return <div className="chartBox"><ResponsiveContainer width="100%" height="100%"><BarChart data={data} margin={{top:12,right:10,left:2,bottom:4}} barCategoryGap="38%"><CartesianGrid vertical={false} stroke="#edf0f2"/><XAxis dataKey="label" axisLine={false} tickLine={false} tick={{fontSize:11,fill:'#606770'}} dy={8}/><YAxis domain={maxValue?[0,maxValue]:undefined} axisLine={false} tickLine={false} width={62} tick={{fontSize:10,fill:'#8b929a'}} tickFormatter={(v)=>unit==='usd'?usd(v):unit==='bp'?bp(v):unit==='count'?Number(v).toLocaleString():unit==='index'?Number(v).toFixed(0):`${Number(v).toFixed(0)}%`}/><Tooltip content={<Tip unit={unit}/>} cursor={{fill:'#f6f7f8'}}/><Bar dataKey="value" radius={[3,3,0,0]} maxBarSize={58}>{data.map((r,i)=><Cell key={i} fill={venueColor(r.label)}/>)}</Bar></BarChart></ResponsiveContainer></div>}

export function HorizontalRanking({data,unit='usd',limit=8,labelWidth=200,maxLabelChars=32}:{data:Row[];unit?:'usd'|'bp';limit?:number;labelWidth?:number;maxLabelChars?:number}){const rows=data.slice(0,limit).reverse();if(!rows.length)return <div className="emptyViz">No reliable data available</div>;return <div className="chartBox rankChart"><ResponsiveContainer width="100%" height="100%"><BarChart data={rows} layout="vertical" margin={{top:6,right:62,left:4,bottom:2}} barCategoryGap="30%"><CartesianGrid horizontal={false} stroke="#edf0f2"/><XAxis type="number" axisLine={false} tickLine={false} tick={{fontSize:10,fill:'#8b929a'}} tickFormatter={(v)=>unit==='usd'?usd(v):bp(v)}/><YAxis type="category" dataKey="label" axisLine={false} tickLine={false} width={labelWidth} interval={0} tick={<SingleLineYAxisTick maxChars={maxLabelChars}/>}/><Tooltip content={<Tip unit={unit}/>} cursor={{fill:'#f6f7f8'}}/><Bar dataKey="value" radius={[0,3,3,0]} maxBarSize={15}>{rows.map((_,i)=><Cell key={i} fill={COLORS[(rows.length-1-i)%COLORS.length]}/>)}</Bar></BarChart></ResponsiveContainer></div>}

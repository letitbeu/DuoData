...
async function getJson<T>(url: string): Promise<T> {
  const r = await fetch(url, {
    ...
    headers: {
      ...
      'User-Agent': 'DuoData/0.4 (+https://github.com/letitbeu/DuoData)',
    },
  })
...
const BINANCE_CLASS: Record<string, string> = {
  ...
  COMMODITY: 'Commodity',
  PREMARKET: 'Pre-market Equity',
  FOREX: 'FX',
  FX: 'FX',
  BOND: 'Bond',
}

async function binance(): Promise<MarketRow[]> {
  const base = 'https://fapi.binance.com'
  const [exchangeInfo, tickers, book, premium] = await Promise.all([
    getJson<any>(`${base}/fapi/v1/exchangeInfo`),
    getJson<any[]>(`${base}/fapi/v1/ticker/24hr`),
    getJson<any[]>(`${base}/fapi/v1/ticker/bookTicker`),
    getJson<any[]>(`${base}/fapi/v1/premiumIndex`),
  ])

  const tickerMap = new Map((Array.isArray(tickers) ? tickers : []).map((x: any) => [x.symbol, x]))
  const bookMap = new Map((Array.isArray(book) ? book : []).map((x: any) => [x.symbol, x]))
  const premiumMap = new Map((Array.isArray(premium) ? premium : []).map((x: any) => [x.symbol, x]))
  const rows: MarketRow[] = []

  for (const i of exchangeInfo?.symbols || []) {
    const underlyingType = String(i.underlyingType || '').toUpperCase()
    if (!BINANCE_TRADFI_TYPES.has(underlyingType)) continue
    if (String(i.status || '').toUpperCase() !== 'TRADING') continue
    const t: any = tickerMap.get(i.symbol)
    if (!t) continue
    const b: any = bookMap.get(i.symbol)
    const p: any = premiumMap.get(i.symbol)
    const last = nullableNum(t.lastPrice)
    if (last === null || last <= 0) continue

    rows.push({
      venue: 'Binance',
      product: `${BINANCE_CLASS[underlyingType] || 'TradFi'} Perpetual`,
      symbol: i.symbol,
      underlying: i.baseAsset || i.pair || i.symbol,
      marketClass: BINANCE_CLASS[underlyingType] || 'TradFi',
      lastPrice: last,
      volume24hUsd: nullableNum(t.quoteVolume),
      // Public aggregate OI is not returned in exchangeInfo/ticker; avoid per-symbol fan-out here.
      openInterestUsd: null,
      // Funding remains excluded from cross-venue ranking until interval normalization is applied.
      fundingRate: null,
      change24h: nullableNum(t.priceChangePercent) === null ? null : num(t.priceChangePercent) / 100,
      bid: nullableNum(b?.bidPrice),
      ask: nullableNum(b?.askPrice),
    })
  }
  return rows
}

async function kraken(): Promise<MarketRow[]> {
  const [tickerResponse, xstockResponse] = await Promise.all([
    getJson<any>('https://futures.kraken.com/derivatives/api/v3/tickers'),
    getJson<any>('https://api.xstocks.fi/api/v2/public/assets'),
  ])

  const tickers = Array.isArray(tickerResponse)
    ? tickerResponse
    : tickerResponse?.tickers || tickerResponse?.result?.tickers || []
  const xassets = Array.isArray(xstockResponse)
    ? xstockResponse
    : xstockResponse?.assets || xstockResponse?.data || xstockResponse?.items || []

  const xSymbols = new Set<string>()
  for (const a of xassets) {
    const s = String(a.symbol || a.ticker || a.identifier || '').toUpperCase()
    if (s) xSymbols.add(s)
  }
  if (!xSymbols.size) throw new Error('xStocks public asset list returned no symbols')

  const rows: MarketRow[] = []
  for (const t of tickers) {
    const symbol = String(t.product_id || t.symbol || '')
    if (!symbol.startsWith('PF_') || !symbol.endsWith('USD')) continue
    const base = symbol.slice(3, -3).toUpperCase()
    if (!xSymbols.has(base)) continue
    const last = nullableNum(t.last ?? t.markPrice)
    if (last === null || last <= 0) continue
    const cleanUnderlying = base.endsWith('X') ? base.slice(0, -1) : base
    ...
    rows.push({
      ...
      marketClass: 'Tokenized Equity / ETF',
      lastPrice: last,
      volume24hUsd: nullableNum(t.volumeQuote),
      // Kraken ticker openInterest unit is venue-native; leave USD OI blank until normalized.
      openInterestUsd: null,
      fundingRate: null,
      // Kraken ticker change is percentage points, e.g. 1.2 = +1.2%.
      change24h: nullableNum(t.change) === null ? null : num(t.change) / 100,
...
const COINBASE_TRADFI_TYPES = new Set([
  'EQUITY', 'EQUITY_ETF', 'EQUITY_INDEX', 'COMMOD', 'COMMOD_INDEX', 'COMMOD_ETF',
])
...
const coverage: CoverageRow[] = [
  { metric: '24h TradFi / RWA trading volume', status: 'LIVE', source: 'Binance + Kraken + Bitget + Bybit + Coinbase INTX public APIs', note: 'Uses venue-reported quote/notional volume only. OKX remains excluded from volume because its derivatives ticker does not publish exact 24h USD quote turnover.' },
  { metric: 'Open interest (USD)', status: 'LIVE', source: 'Bybit + Bitget + OKX + Coinbase INTX public APIs', note: 'Direct USD OI where published; Bitget and Coinbase quantities are converted using venue mark price and multiplier. Binance/Kraken OI is left blank until a normalized USD field is implemented.' },
  { metric: 'Funding rate', status: 'PARTIAL', source: 'Bybit + Bitget public APIs', note: 'Current rates are shown only where units/intervals are already compatible. Binance, Kraken, OKX and Coinbase funding will be normalized before inclusion.' },
  { metric: 'Best bid / ask & top-of-book spread', status: 'LIVE', source: 'Binance + Kraken + Bitget + Bybit + OKX + Coinbase INTX public APIs', note: 'Current L1 quote. Deeper liquidity metrics are not inferred from L1.' },
  { metric: 'TradFi instrument coverage', status: 'LIVE', source: 'Venue-native metadata + xStocks public asset registry', note: 'Uses Binance underlyingType, Kraken xStocks registry match, Bybit symbolType, Bitget isRwa, OKX instCategory and Coinbase underlying_type. No synthetic instruments are created.' },
  ...
  { metric: '±10/25/50bp depth & $100k slippage', status: 'PARTIAL', source: 'Public order-book / Kraken analytics APIs', note: 'Technically available for several venues; aggregation will be enabled after rate-limit-safe collectors are added.' },
  { metric: 'Cross-venue tracking error', status: 'PARTIAL', source: 'Exchange index/mark + independent reference market', note: 'Exchange marks are public; a consolidated cash-equity reference feed still needs to be selected.' },
  ...
  { metric: 'TradFi Penetration Ratio (TradFi/Crypto)', status: 'PARTIAL', source: 'Exchange public market data', note: 'TradFi leg is available on multiple venues; denominator needs a rigorously normalized exchange-wide crypto turnover series.' },
...
export async function getSnapshot(): Promise<Snapshot> {
  const errors: string[] = []
  const sources = [
    { venue: 'Binance', promise: binance() },
    { venue: 'Kraken', promise: kraken() },
